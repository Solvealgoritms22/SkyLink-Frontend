            ...
            <p-tabView styleClass="custom-tabview">
                ...
                <p-tabPanel header="Estructura">
                                ...
                                <tbody>
                                    <tr *ngFor="let column of viewColumns" class="hover:bg-surface-700">
                                        ...
                                        <td class="border border-surface-500 p-2">
                                            <span *ngIf="column.sourceTable && column.sourceColumn" class="font-mono text-xs"> {{ column.sourceTable }}.{{ column.sourceColumn }} </span>
                                            <span *ngIf="!column.sourceTable || !column.sourceColumn" class="text-gray-500 italic"> Expresión calculada </span>
                                        </td>
                    ...
                    <div *ngIf="viewSourceTables && viewSourceTables.length > 0" class="mb-6">
                        <h4 class="text-xl font-medium text-slate-200 mb-3">Tablas Fuente:</h4>
                        <div *ngFor="let table of viewSourceTables; let i = index" class="mb-3 bg-surface-700 p-4 rounded-lg">
                            <div class="flex items-center gap-2 mb-2">
                                <span class="font-semibold font-mono text-slate-200">{{ table.name }}</span>
                            ...
                            <div *ngIf="table.joinCondition" class="bg-surface-600 p-2 rounded mt-1">
                                <p class="text-xs font-mono text-slate-200">{{ table.joinCondition }}</p>
                            </div>
                        </div>
                    </div>
                    <!-- Filtros aplicados -->
                    <div *ngIf="viewFilterConditions && viewFilterConditions.length > 0" class="mb-6">
                        <h4 class="text-xl font-medium text-slate-200 mb-3">Filtros Aplicados:</h4>
                        <div *ngFor="let filter of viewFilterConditions; let i = index" class="mb-2">
                            <div class="bg-surface-700 p-3 rounded">
                                <p class="font-mono text-sm text-slate-200">{{ filter }}</p>
                            </div>
                        </div>
                    </div>
                </p-tabPanel>
                ...
                <p-tabPanel header="Rendimiento" *ngIf="viewPerformanceNotes">
                    <div class="mb-6">
                        <h4 class="text-xl font-medium text-slate-200 mb-2">Consideraciones de Rendimiento:</h4>
