                    ...
                    <tbody>
                      <tr *ngFor="let perm of role.permissions" class="hover:bg-surface-200">
                        ...
                        <td class="border border-surface-500 p-2">
                          <div class="flex flex-wrap gap-1">
                            <p-chip *ngFor="let priv of perm.privileges" 
                              [label]="priv" 
                              [styleClass]="
                                priv === 'SELECT' ? 'bg-blue-600 text-xs' : 
                                priv === 'INSERT' ? 'bg-green-600 text-xs' : 
                                priv === 'UPDATE' ? 'bg-orange-600 text-xs' : 
                                priv === 'DELETE' ? 'bg-red-600 text-xs' :
                                priv === 'EXECUTE' ? 'bg-purple-600 text-xs' : 'bg-gray-600 text-xs'">
                            </p-chip>
                          </div>
                        </td>
                      </tr>
...
export class SecuritySectionComponent {
  ...
  get securityPasswordPolicy(): string[] {
    return this.securityDetail?.passwordPolicy ? 
      this.securityDetail.passwordPolicy.details : 
      this.passwordPolicy;
  }
