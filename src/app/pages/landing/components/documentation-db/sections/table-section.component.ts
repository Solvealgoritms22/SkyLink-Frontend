    ...
    <section class="mb-12 bg-surface-800 rounded-xl p-6 shadow-lg">      <h2 class="text-primary text-2xl font-semibold mb-4">
      ...
      <p-tabView styleClass="custom-tabview">
        ...
        <p-tabPanel header="Estructura">
          <div class="mb-8">
            <h3 class="text-lg font-medium text-secondary-400 mb-3">Diccionario de Datos</h3>
          ...
          <div *ngIf="tableRelations && tableRelations.length > 0" class="mb-8">
            <h3 class="text-lg font-medium text-secondary-400 mb-3">Relaciones</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div *ngFor="let relation of tableRelations" class="bg-surface-700 p-4 rounded-lg border-l-4"
                   [ngClass]="{'border-green-500': relation.type === 'one-to-one', 
                ...
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <span class="text-xs px-2 py-1 rounded bg-surface-600">
                    {{ relation.type === 'one-to-one' ? '1:1' : 
                       ...
                       relation.type === 'many-to-one' ? 'N:1' : 'N:M' }}
                  </span>
                </div>
                <p class="text-sm text-slate-300 mb-2">{{ relation.description }}</p>
                <div class="text-xs">
                  <div *ngFor="let col of relation.columns" class="flex items-center gap-1">                    <span class="font-mono">{{ getTableName }}.{{ col.local }}</span>
                    <span>→</span>
          ...
          </div>          <!-- Índices si existen -->
          <div *ngIf="tableIndexes && tableIndexes.length > 0" class="mb-8">
            <h3 class="text-lg font-medium text-secondary-400 mb-3">Índices</h3>
            <div class="overflow-x-auto">
              <table class="w-full border-collapse table-auto">
                <thead>
                  <tr class="bg-surface-600">
                ...
                <tbody>
                  <tr *ngFor="let index of tableIndexes" class="hover:bg-surface-700">
