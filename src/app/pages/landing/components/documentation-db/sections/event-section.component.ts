            ...
            <p-tabView styleClass="custom-tabview">
                ...
                <p-tabPanel header="Detalles">
                        ...
                        <div class="bg-surface-700 p-4 rounded-lg">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p-chip [label]="eventSchedule.type" [styleClass]="eventSchedule.type === 'ONE TIME' ? 'bg-purple-600' : 'bg-blue-600'"> </p-chip>
                                </div>
                                <div>
                            ...
                            <div *ngIf="eventScheduleDescription" class="mt-4">
                    ...
                    <div *ngIf="eventAffectedTables && eventAffectedTables.length > 0" class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div
                                *ngFor="let table of eventAffectedTables"
                            ...
                            >
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <p-chip [label]="table.operation" [styleClass]="table.operation === 'INSERT' ? 'bg-green-600' : table.operation === 'SELECT' ? 'bg-blue-600' : table.operation === 'UPDATE' ? 'bg-orange-600' : 'bg-red-600'">
                                    </p-chip>
                                </div>
                        ...
                        <ng-template pTemplate="content" let-event>
                            <div class="bg-surface-700 p-4 rounded-lg mb-3">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    <p-chip [label]="event.status" [styleClass]="event.status === 'SUCCESS' ? 'bg-green-600' : event.status === 'FAILED' ? 'bg-red-600' : 'bg-gray-600'"> </p-chip>
                                </div>
    ...
    get eventExecutionHistory(): EventExecutionHistory[] {
        if (this.eventDetail?.executionHistory) {
            return this.eventDetail.executionHistory.map((history) => ({
                date: history.date,
                status: history.status === 'SUCCESS' ? 'SUCCESS' : history.status === 'FAILURE' ? 'FAILED' : 'SKIPPED',
