    ...
    <section class="mb-12 bg-surface-800 rounded-xl p-6 shadow-lg">      <h2 class="text-primary text-2xl font-semibold mb-4">
      ...
      <p-tabView styleClass="custom-tabview">
        ...
        <p-tabPanel header="Detalles">
          <!-- Información básica -->
          <div class="mb-6">
            <h4 class="text-xl font-medium text-secondary-400 mb-3">Información Básica:</h4>
            <div class="bg-surface-700 p-4 rounded-lg">              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p class="text-md font-medium text-gray-300">Tabla:</p>
                  <p class="text-lg font-mono">{{ triggerDetailsInfo.table }}</p>
                </div>
                <div>
                  <p class="text-md font-medium text-gray-300">Temporización:</p>
                  <p-chip [label]="triggerDetailsInfo.timing" 
                          [styleClass]="triggerDetailsInfo.timing === 'BEFORE' ? 'bg-purple-600' : 'bg-blue-600'">
                  </p-chip>
                </div>
                <div>
                  <p class="text-md font-medium text-gray-300">Evento:</p>
                  <p-chip [label]="triggerDetailsInfo.event" 
                          [styleClass]="triggerDetailsInfo.event === 'INSERT' ? 'bg-green-600' : 
                                      triggerDetailsInfo.event === 'UPDATE' ? 'bg-orange-600' : 'bg-red-600'">
                  </p-chip>
                </div>
                <div>
                  <p class="text-md font-medium text-gray-300">Alcance:</p>
                  <p-chip [label]="triggerDetailsInfo.perRow ? 'Por Fila (FOR EACH ROW)' : 'Por Sentencia'" 
                          styleClass="bg-gray-600">
                  </p-chip>
                </div>
              </div>
            </div>
          </div>          <!-- Variables Especiales -->
          <div *ngIf="triggerSpecialVariables && triggerSpecialVariables.length > 0" class="mb-6">
            <h4 class="text-xl font-medium text-secondary-400 mb-3">Variables Especiales:</h4>
          ...
          </div>          <!-- Operaciones en Tablas -->
          <div *ngIf="triggerTableOps && triggerTableOps.length > 0" class="mb-6">
            <h4 class="text-xl font-medium text-secondary-400 mb-2">Operaciones en Tablas:</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div *ngFor="let op of triggerTableOps" class="bg-surface-700 p-4 rounded-lg border-l-4"
                   [ngClass]="{
                ...
                <div class="flex items-center gap-2 mb-2">
                  <span class="font-semibold">{{ op.tableName }}</span>
                  <p-chip [label]="op.operation" 
                         [styleClass]="op.operation === 'INSERT' ? 'bg-green-600' : 
                                      op.operation === 'SELECT' ? 'bg-blue-600' : 
                                      op.operation === 'UPDATE' ? 'bg-orange-600' : 'bg-red-600'">
                  </p-chip>
                </div>
          ...
          <div *ngIf="triggerUseCases && triggerUseCases.length > 0" class="mb-6">
            <h4 class="text-xl font-medium text-secondary-400 mb-2">Casos de Uso:</h4>
          ...
          <div *ngIf="triggerImplementationNotes" class="mb-6">            <h4 class="text-xl font-medium text-secondary-400 mb-2">Notas de Implementación:</h4>
        ...
        <!-- Ejemplos -->
        <p-tabPanel header="Ejemplos" *ngIf="triggerExamples && triggerExamples.length > 0">          <div *ngFor="let example of triggerExamples; let i = index" class="mb-6">
            <h3 class="text-md font-medium text-secondary-300 mb-2">{{ example.description }}</h3>
            <div class="bg-surface-700 p-4 rounded-lg mb-3">
              <h4 class="text-sm font-medium text-secondary-300 mb-2">Operación que activa el trigger:</h4>
              <app-code-card language="sql" [code]="example.triggeringSql" (copy)="onCopy(example.triggeringSql)"></app-code-card>
            </div>
            <div class="bg-surface-700 p-4 rounded-lg">
              <h4 class="text-sm font-medium text-secondary-300 mb-2">Resultado:</h4>
...
export class TriggerSectionComponent {  @Input() triggerName = '';
  @Input() description = '';
  @Input() sqlCode = '';
