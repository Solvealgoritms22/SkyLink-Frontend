    ...
    <section class="mb-12 bg-surface-800 rounded-xl p-6 shadow-lg">      <h2 class="text-primary text-2xl font-semibold mb-4">
      ...
      <p-tabView styleClass="custom-tabview">
        ...
        <p-tabPanel header="Detalles">
          <!-- Parámetros -->
          <div class="mb-6">
            <h4 class="text-xl font-medium text-secondary-400 mb-2">Parámetros:</h4>
            <div *ngIf="procedureParameters && procedureParameters.length > 0; else noParameters" class="overflow-x-auto rounded-md shadow">
              <table class="w-full border-collapse table-auto">
                ...
                <tbody>
                  <tr *ngFor="let param of procedureParameters" class="hover:bg-surface-700">
                    <td class="border border-surface-500 p-2 font-mono">{{ param.name }}</td>
                    <td class="border border-surface-500 p-2">
                      <p-chip [label]="param.direction" 
                              [styleClass]="param.direction === 'IN' ? 'bg-green-600' : 
                                          param.direction === 'OUT' ? 'bg-blue-600' : 'bg-purple-600'" 
                              [pTooltip]="param.direction === 'IN' ? 'Parámetro de entrada' : 
                                        param.direction === 'OUT' ? 'Parámetro de salida' : 'Parámetro de entrada/salida'">
                      </p-chip>
                    </td>
          ...
          <div *ngIf="procedureTableOperations && procedureTableOperations.length > 0" class="mb-6">            <h4 class="text-xl font-medium text-secondary-400 mb-2">Operaciones en Tablas:</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div *ngFor="let op of procedureTableOperations" class="bg-surface-700 p-4 rounded-lg border-l-4"
                   [ngClass]="{
          ...
          <div *ngIf="procedureTransactionInfo" class="mb-6">            <h4 class="text-xl font-medium text-secondary-400 mb-2">Información de Transacción:</h4>
            <div class="bg-surface-700 p-4 rounded-lg">
              ...
              <div *ngIf="procedureTransactionInfo.hasTransaction">
                ...
                <p class="text-sm whitespace-pre-line">{{ procedureTransactionInfo.errorHandling }}</p>
              </div>
            </div>
          </div>

          <!-- Notas de Implementación -->
          <div *ngIf="procedureImplementationNotes" class="mb-6">            <h4 class="text-xl font-medium text-secondary-400 mb-2">Notas de Implementación:</h4>
            <div class="bg-surface-700 p-4 rounded-lg">
        ...
        <!-- Ejemplos de uso -->
        <p-tabPanel header="Ejemplos de Uso" *ngIf="procedureExampleUsages && procedureExampleUsages.length > 0">          <div *ngFor="let example of procedureExampleUsages; let i = index" class="mb-6">
